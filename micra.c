...
int editorReadKey() {
  ...
  if (c == '\x1b') {
    char seq[3];  // Handling longer escape sequences in the future
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (MICRA_TAB_STOP - 1) - (rx % MICRA_TAB_STOP);
    rx++;
  }
  return rx;
}
...
void editorUpdateRow(erow *row) {
  ...
  free(row->render);
  row->render = malloc(row->size + tabs*(MICRA_TAB_STOP - 1) + 1);

  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % MICRA_TAB_STOP != 0) row->render[idx++] = ' ';
    } else {
